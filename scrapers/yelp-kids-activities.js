...
const CATEGORY = 'Kids Activities & Classes';
const LOCATIONS = [
  { city: 'Plano', query: 'find_desc=kids+activities+classes&find_loc=Plano%2C+TX' },
  { city: 'Frisco', query: 'find_desc=kids+activities+classes&find_loc=Frisco%2C+TX' }
];
const MAX_PAGES = 3;
...
async function scrape() {
  ...
  try {
    for (const loc of LOCATIONS) {
      console.log(`\n[${loc.city}]`);
      const results = await scrapeYelp(context, loc);
